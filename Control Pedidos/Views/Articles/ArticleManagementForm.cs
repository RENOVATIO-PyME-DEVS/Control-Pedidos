...
namespace Control_Pedidos.Views.Articles
{
    public partial class ArticleManagementForm : Form
    {
        ...
        public ArticleManagementForm(DatabaseConnectionFactory connectionFactory)
        {
            ...
            typeComboBox.DataSource = new List<KeyValuePair<string, string>>
            {
                new KeyValuePair<string, string>("N", "Normal"),
                new KeyValuePair<string, string>("K", "Kit"),
                new KeyValuePair<string, string>("P", "Proceso")
            };
            typeComboBox.DisplayMember = "Value";
            typeComboBox.ValueMember = "Key";
            typeComboBox.SelectedValue = "N";
        ...
        private void ConfigureGrids()
        {
            ...
            articlesGrid.Columns.Add(new DataGridViewTextBoxColumn
            {
                DataPropertyName = nameof(Articulo.TipoDescripcion),
                HeaderText = "Tipo",
                Width = 80
            });
            ...
            articlesGrid.Columns.Add(new DataGridViewTextBoxColumn
            {
                DataPropertyName = nameof(Articulo.EstatusDescripcion),
                HeaderText = "Estatus",
                Width = 90
            });
        ...
        private void LoadComponentCatalog()
        {
            try
            {
                _catalogoComponentes = Articulo.Listar(_connectionFactory, string.Empty)
                    .Where(a => !a.EsKit && string.Equals(a.Estatus, "N", StringComparison.OrdinalIgnoreCase))
                    .ToList();
        ...
        private Articulo BuildArticuloFromForm()
        {
            var articulo = new Articulo
            {
                ...
                TipoArticulo = typeComboBox.SelectedValue?.ToString() ?? "N",
        ...
        private bool ValidateForm()
        {
            ...
            if (string.Equals(typeComboBox.SelectedValue?.ToString(), "K", StringComparison.OrdinalIgnoreCase) && _componentes.Count == 0)
            {
        ...
        private void articlesGrid_SelectionChanged(object sender, EventArgs e)
        {
            if (articlesGrid.CurrentRow?.DataBoundItem is Articulo articulo)
            {
                ...
                var tipo = string.IsNullOrWhiteSpace(articulo.TipoArticulo) ? "N" : articulo.TipoArticulo;
                typeComboBox.SelectedValue = tipo;
                ...
                var estatus = string.IsNullOrWhiteSpace(articulo.Estatus) ? "N" : articulo.Estatus;
        ...
        {
            if (!string.Equals(typeComboBox.SelectedValue?.ToString(), "K", StringComparison.OrdinalIgnoreCase))
            {
        ...
        {
            var esKit = string.Equals(typeComboBox.SelectedValue?.ToString(), "K", StringComparison.OrdinalIgnoreCase);
