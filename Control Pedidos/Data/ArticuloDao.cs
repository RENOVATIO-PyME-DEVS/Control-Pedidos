...
VALUES (@nombre, @nombreCorto, @tipoArticulo, @unidadMedida, @unidadControl, @contenidoControl, @precio, @fechaPrecio, @usuarioPrecioId, @estatus, @tiene);";
                        ...
                        using (var command = new MySqlCommand(insertQuery, connection, transaction))
                        {
                            ...
                            command.Parameters.AddWithValue("@tipoArticulo", string.IsNullOrWhiteSpace(articulo.TipoArticulo) ? "N" : articulo.TipoArticulo);
                            ...
                            command.Parameters.AddWithValue("@estatus", string.IsNullOrWhiteSpace(articulo.Estatus) ? "N" : articulo.Estatus);
                            command.Parameters.AddWithValue("@tiene", articulo.TieneInventario ? "S" : "N");
...
WHERE articulo_id = @articuloId;";
                        ...
                        const string deleteDetalle = "DELETE FROM articulos_kit WHERE articulo_id = @kitId";
...
WHERE (@filtro = '' OR nombre LIKE CONCAT('%', @filtro, '%'))";
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var articulo = new Articulo
                            {
                                ...
                                TipoArticulo = reader.GetString("tipo_articulo").ToUpperInvariant(),
                                ...
                                TieneInventario = !reader.IsDBNull(reader.GetOrdinal("tiene")) && EsValorVerdadero(reader.GetString("tiene"))
                            };
        ...
        private void GuardarComponentes(MySqlConnection connection, MySqlTransaction transaction, Articulo articulo)
        {
            ...
            const string insertDetalle = "INSERT INTO articulos_kit (articulo_id, articulo_compuesto_id, cantidad) VALUES (@kitId, @articuloId, @cantidad)";
        ...
        private void CargarComponentes(IList<Articulo> kits)
        {
            ...
            var query = $@"SELECT ak.articulo_id, ak.articulo_compuesto_id, ak.cantidad, a.nombre, a.nombre_corto
FROM articulos_kit ak
INNER JOIN articulos a ON a.articulo_id = ak.articulo_compuesto_id
WHERE ak.articulo_id IN ({ids})";
        ...
        private static bool EsValorVerdadero(string valor)
        {
